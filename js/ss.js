...
document.querySelector('.lanjbox').addEventListener('click', function() {
    document.querySelector('.lanjbox').style.display = ('none')
})

//форма Записать на консультацию
document.querySelector('.consultation-form').addEventListener('submit', function(evt) {
    evt.preventDefault();
...
let play = document.querySelectorAll('.play-video')
...
for(let i = 0; i < play.length; i++) {
    play[i].addEventListener('click', function() {
        let plet = play[i].dataset.video
        console.log(plet)
        document.querySelector('.lanjbox').style.display = ('flex')
        document.querySelector('.lanjbox').innerHTML = plet
    })
}